    ...
    >
      <motion.div variants={itemVariants}>
        <PageHeader
          ...
          badge={
            <span className="inline-flex items-center rounded-md bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-700 dark:text-emerald-400">
              Free for MMSS Mohali
            </span>
          }
        />
      </motion.div>
      ...
      <section className="space-y-4" aria-label="Contributors">
        {DEVELOPERS.map((dev) => (
          ...
          >
            <div
              className={cn(
                'pointer-events-none absolute inset-0 bg-gradient-to-br opacity-50 group-hover:opacity-100 transition-opacity duration-500', 
                dev.accentClass
              )}
              aria-hidden
            />
            <div className="relative flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:gap-5 sm:p-6">
              ...
              <div className="min-w-0 flex-1 space-y-3">
                <div>
                  <h2 className="text-base font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 group-hover:text-neutral-950 dark:group-hover:text-white transition-colors">
                    {dev.name}
                  </h2>
      ...
      <motion.section
        variants={itemVariants}
        className={cn(
          'rounded-2xl border border-dashed border-neutral-300/80 dark:border-neutral-700',
          'bg-neutral-50/80 dark:bg-neutral-900/40 px-5 py-4 sm:px-6'
        )}
      >
        <p className="text-[13px] leading-relaxed text-neutral-600 dark:text-neutral-300">
          This app is given to the school at no cost. If you like what we build, say hi on X or GitHub —
          connections help us keep shipping for MMSS.
        </p>
        <motion.a
          ...
          rel="noopener noreferrer"
          whileHover={{ x: 4 }}
          className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-neutral-800 dark:text-neutral-200 hover:text-neutral-950 dark:hover:text-white transition-colors"
        >
