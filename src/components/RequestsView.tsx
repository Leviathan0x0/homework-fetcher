...
function getCategoryIcon(cat: string | null | undefined) {
  switch (cat) {
    ...
    default: return <Reicon name="message-square" size={16} />;
  }
}
                ...
                <div className="flex items-center gap-1">
                  ...
                  {!item.isOwner && item.status === 'open' && item.creatorUserId && (
                    <button onClick={() => handleHelp(item)} disabled={helpingId === item.id}
                      ...
                      {helpingId === item.id ? <Reicon name="loader" size={12} isLoading className="animate-spin" /> : <Reicon name="message-square" size={12} preset="bounce" className="w-3 h-3" />}
                      <span>Help</span>
                    </button>
                  )}
                  {item.isOwner && (
                    <button onClick={() => handleDelete(item.id)} disabled={deletingId === item.id}
                      ...
                      title="Delete">
                      {deletingId === item.id ? <Reicon name="loader" size={14} isLoading className="animate-spin" /> : <Reicon name="trash-2" size={14} />}
                    </button>
                  )}
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-3 flex items-center justify-end gap-2 border-t border-neutral-100 dark:border-neutral-800">
                ...
                <button type="submit" disabled={isSubmitting || !formTitle.trim() || !formContent.trim()}
                  ...
                  {isSubmitting ? <><Reicon name="loader" size={14} isLoading className="animate-spin" /><span>Posting...</span></> : <><Reicon name="heart-handshake" size={14} preset="scale" className="w-3.5 h-3.5" /><span>Post Request</span></>}
                </button>
              </div>
            </form>
          </div>
