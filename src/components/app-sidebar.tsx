...
export function AppSidebar({
  ...
  const navMain = [
    ...
    { id: "classwork" as ViewType, title: "Classwork", icon: UploadCloud },
    { id: "requests" as ViewType, title: "Requests", icon: Handshake },
    { id: "messages" as ViewType, title: "Messages", icon: MessageCircle },
    { id: "calendar" as ViewType, title: "Calendar", icon: CalendarDays },
    { id: "exams" as ViewType, title: "Exams", icon: GraduationCap },
    { id: "recent" as ViewType, title: "Recent", icon: Clock },
    { id: "all" as ViewType, title: "All homework", icon: Layers },
    { id: "attachments" as ViewType, title: "Attachments", icon: Paperclip },
    { id: "completed" as ViewType, title: "Completed", icon: CheckCircle2 },
  ];
  ...
  return (
    <Sidebar collapsible="icon" {...props}>
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="text-[11px] font-normal text-muted-foreground group-data-[collapsible=icon]:hidden">
            Navigation
          </SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {navMain.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.id}>
                    ...
                    >
                      <Icon className="size-4 shrink-0 transition-transform duration-200 group-hover/sbitem:-rotate-6" />
                      <span className="group-data-[collapsible=icon]:hidden">{item.title}</span>
                      {item.badge !== undefined && (
                        <span className="ml-auto flex size-4.5 items-center justify-center rounded-full bg-primary/10 text-[10px] font-semibold text-primary group-data-[collapsible=icon]:hidden">
                          {item.badge}
                        </span>
                      )}
                    </SidebarMenuButton>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              <SidebarMenuItem>
                ...
                >
                  <Settings className="size-4 shrink-0 transition-transform duration-300 group-hover/sbset:rotate-45" />
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            ...
            <div className="p-2.5 flex flex-col gap-2 rounded-2xl bg-sidebar-accent/40 text-sidebar-accent-foreground text-xs border border-sidebar-border/40 group-data-[collapsible=icon]:hidden">
              <div className="flex items-center justify-between">
                ...
                {sessionStatus === "connected" ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[10px] text-amber-600 dark:text-amber-400 font-medium">
                    <AlertTriangle className="size-3 animate-wiggle-subtle" /> Expired
                  </span>
                )}
              </div>
              <button
              ...
              >
                <LogOut className="size-3 transition-transform duration-200 group-hover/sblogout:-translate-x-0.5" />
                <span>Sign out</span>
              </button>
            </div>
            ...
            <div className="hidden group-data-[collapsible=icon]:flex items-center justify-center p-1">
              ...
              >
                <LogOut className="size-4 shrink-0 transition-transform duration-200 group-hover/sbiconlogout:-translate-x-0.5" />
              </SidebarMenuButton>
            </div>
          </SidebarMenuItem>
        </SidebarMenu>
